...
esp_err_t mq2_init()
{
    ...
    esp_err_t error_2 = adc1_config_channel_atten(ADC2_CHANNEL_7, ADC_ATTEN_DB_11); //cấu hình độ suy giảm của kênh ADC kênh 0 được cấu hình với độ suy giảm 11 dB chỉnh khoảng điện áp đầu vào của ADC để phù hợp với tín hiệu đầu vào.
    ESP_ERROR_CHECK_WITHOUT_ABORT(error_2);
    esp_err_t error_3 = esp_adc_cal_characterize(ADC_UNIT_1, ADC_ATTEN_DB_11, ADC_WIDTH_BIT_12, DEFAULT_VREF, &adc2_chars);//Hàm này lấy các tham số như đơn vị ADC, độ suy giảm, độ phân giải và điện áp tham chiếu mặc định, và sau đó lưu trữ thông số hiệu chuẩn vào biến adc_chars
...
esp_err_t mq2_reading(uint32_t* co)
{
    // esp_adc_cal_characteristics_t adc_chars;
        ...
        for (int i = 0; i < 50; i++)
        {
            adc_reading += adc1_get_raw(ADC2_CHANNEL_7);
